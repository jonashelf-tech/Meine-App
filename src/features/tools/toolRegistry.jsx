import { lazy } from 'react'

// ─── SVG icon paths (Lucide-style, viewBox 0 0 24 24, stroke-based) ───────────
const ICONS = {
  rad:          { el: (s) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><polyline points="16 3 21 3 21 8"/><line x1="4" y1="20" x2="21" y2="3"/><polyline points="21 16 21 21 16 21"/><line x1="15" y1="15" x2="21" y2="21"/></svg> },
  timer:        { el: (s) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg> },
  rezepte:      { el: (s) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M3 2v7c0 1.1.9 2 2 2h4a2 2 0 0 0 2-2V2"/><line x1="7" y1="2" x2="7" y2="22"/><path d="M21 15V2a5 5 0 0 0-5 5v6c0 1.1.9 2 2 2h3zm0 0v7"/></svg> },
  pizza:        { el: (s) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M15 11h.01"/><path d="M11 15h.01"/><path d="M16 16h.01"/><path d="m2 16 20 6-6-20A20 20 0 0 0 2 16"/><path d="M5.71 17.11a17.04 17.04 0 0 1 11.4-11.4"/></svg> },
  elvi:         { el: (s) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M10.5 3.5a5 5 0 0 1 7.07 7.07l-7.07 7.07a5 5 0 0 1-7.07-7.07l7.07-7.07z"/><line x1="8.5" y1="8.5" x2="15.5" y2="15.5"/></svg> },
  fitness:      { el: (s) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="m16 16 3-8 3 8c-.87.65-1.92 1-3 1s-2.13-.35-3-1Z"/><path d="m2 16 3-8 3 8c-.87.65-1.92 1-3 1s-2.13-.35-3-1Z"/><path d="M7 21h10"/><path d="M12 3v18"/><path d="M3 7h2c2 0 5-1 7-2 2 1 5 2 7 2h2"/></svg> },
  geburtstage:  { el: (s) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="10" width="20" height="12" rx="2"/><path d="M12 10V6"/><path d="M8 10V8"/><path d="M16 10V8"/><path d="M2 15h20"/></svg> },
  garten:       { el: (s) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M12 5a3 3 0 1 1 3 3m-3-3a3 3 0 1 0-3 3m3-3v1M9 8a3 3 0 1 0 3 3M9 8h1m5 0a3 3 0 1 1-3 3m3-3h-1m-2 3v-1"/><circle cx="12" cy="8" r="2"/><path d="M12 10v12"/><path d="M12 22c4.2 0 7-1.667 7-5-4.2 0-7 1.667-7 5Z"/><path d="M12 22c-4.2 0-7-1.667-7-5 4.2 0 7 1.667 7 5Z"/></svg> },
  reminder:     { el: (s) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg> },
  haushalt:     { el: (s) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg> },
  wasjetzt:     { el: (s) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg> },
  klaeren:      { el: (s) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polygon points="16.24 7.76 14.12 14.12 7.76 16.24 9.88 9.88 16.24 7.76"/></svg> },
  kognitiv:     { el: (s) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2a10 10 0 0 1 10 10c0 5.52-4.48 10-10 10S2 17.52 2 12"/><circle cx="12" cy="12" r="3"/><path d="M12 9V5M15 12h4"/></svg> },
  projekte:     { el: (s) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/><line x1="9" y1="14" x2="15" y2="14"/><line x1="12" y1="11" x2="12" y2="17"/></svg> },
  growth:       { el: (s) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22V12"/><path d="M12 12C12 8 9 6 4 6c0 4 3 6 8 6z"/><path d="M12 11c0-3.5 2.5-6 7-6 0 3.5-2.5 6-7 6z"/></svg> },
  notizen:      { el: (s) => <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/><line x1="9" y1="13" x2="15" y2="13"/><line x1="9" y1="17" x2="13" y2="17"/></svg> },
}

export function ToolIcon({ id, size = 20 }) {
  const icon = ICONS[id]
  if (!icon) return null
  return icon.el(size)
}

export const TOOL_REGISTRY = [
  {
    id: 'geburtstage',
    tabId: 4,
    name: 'Geburtstage',    color: '#FF2D78',
    description: 'Geburtstage & Erinnerungen',
    featured: true,
    intro: 'Trag Geburtstage einmal ein — die App erinnert dich rechtzeitig und blendet sie im Kalender ein. Optional mit Geschenk-Vorlauf, damit nichts kurzfristig wird.',
    standalone: true,
    integrated: true,
    component: lazy(() => import('./geburtstage/TabGeburtstage')),
  },
  {
    id: 'timer',
    tabId: 5,
    name: 'Fokus-Timer',    color: '#00CFFF',
    description: 'Pomodoro & Fokus-Blöcke',
    standalone: true,
    integrated: false,
    component: lazy(() => import('./timer/TabTimer')),
  },
  {
    id: 'rezepte',
    tabId: 6,
    name: 'Mealprep',    color: '#FF9F43',
    description: 'Rezepte, Einkauf, Vorräte',
    featured: true,
    intro: 'Rezepte, automatische Einkaufsliste und dein Tiefkühl-Vorrat. Plan ein Menü, hak beim Kochen ab, der Rest wird für dich zusammengerechnet.',
    standalone: true,
    integrated: false,
    component: lazy(() => import('./rezepte/TabRezepte')),
  },
  {
    id: 'pizza',
    tabId: 7,
    name: 'Pizza-Rechner',    color: '#EF4444',
    description: 'Teig-Rechner & Zeitplan',
    standalone: true,
    integrated: false,
    component: lazy(() => import('./pizza/TabPizza')),
  },
  {
    id: 'elvi',
    tabId: 8,
    name: 'Elvi',    color: '#14B8A6',
    description: 'Methylphenidat Dosisplanung',
    standalone: true,
    integrated: false,
    component: lazy(() => import('./elvi/TabElvi')),
  },
  {
    id: 'fitness',
    tabId: 9,
    name: 'Fitness',    color: '#22C55E',
    description: 'Krafttraining, Pläne & Körpergewicht',
    featured: true,
    intro: 'Kraftpläne, geführte Trainings mit Live-Session und dein Körpergewicht an einem Ort. Ein kurzer Coach fragt dich am Anfang ab und baut dir einen passenden Plan.',
    standalone: true,
    integrated: true,
    component: lazy(() => import('./fitness/TabFitness')),
  },
  {
    id: 'garten',
    tabId: 10,
    name: 'Garten',    color: '#2DD4BF',
    description: 'Dein Begleiter — wächst mit allem, was du erledigst',
    featured: true,
    intro: 'Dein stiller Begleiter: Alles, was du erledigst, lässt einen kleinen Garten wachsen. Kein Zwang, kein Verlieren — nur ein sichtbares Zeichen für deine Tage.',
    standalone: true,
    integrated: false,
    component: lazy(() => import('./garten/TabGarten')),
  },
  {
    id: 'rad',
    tabId: 11,
    name: 'Zufallsrad',    color: '#A855F7',
    description: 'Zufälliges Todo auswählen',
    standalone: true,
    integrated: false,
    component: lazy(() => import('./rad/TabRad')),
  },
  {
    id: 'reminder',
    tabId: 12,
    name: 'Reminder',    color: '#6366F1',
    description: 'Selfcare-Erinnerungen & Routinen',
    standalone: true,
    integrated: true,
    component: lazy(() => import('./reminder/TabReminder')),
  },
  {
    id: 'haushalt',
    tabId: 13,
    name: 'Haushalt',    color: '#3B82F6',
    description: 'Neurodivergenzfreundliche Haushaltsroutinen',
    featured: true,
    intro: 'Haushalt in ruhigen Routinen statt großer Listen — nach Räumen sortiert, im Tempo, das gerade passt. Fällige Aufgaben landen als Karte im Tagesplaner.',
    standalone: true,
    integrated: true,
    component: lazy(() => import('./haushalt/TabHaushalt')),
  },
  {
    id: 'wasjetzt',
    tabId: 14,
    name: 'Was jetzt?',    color: '#FBBF24',
    description: 'Top 3 Aufgaben für dein Zeitfenster',
    standalone: true,
    integrated: false,
    component: lazy(() => import('./wasjetzt/TabWasJetzt')),
  },
  {
    id: 'klaeren',
    tabId: 15,
    name: 'Prokrastination',    color: '#34D399',
    description: 'Alte Todos entstauben und wieder ins Rollen bringen',
    standalone: true,
    integrated: false,
    component: lazy(() => import('./klaeren/TabKlaeren')),
  },
  {
    id: 'kognitiv',
    tabId: 16,
    name: 'Kognitiv',    color: '#8B5CF6',
    description: 'Kognitives Training: Reaktion, Aufmerksamkeit, Gedächtnis',
    featured: true,
    intro: 'Kurze tägliche Übungen für Reaktion, Aufmerksamkeit und Gedächtnis — als eine zusammenhängende Einheit. Du stellst dir zusammen, was drankommt, der Rest läuft geführt.',
    standalone: true,
    integrated: true,
    component: lazy(() => import('./kognitiv/TabKognitiv')),
  },
  {
    id: 'projekte',
    tabId: 17,
    name: 'Projekte',    color: '#84CC16',
    description: 'Kategorien als Projekte mit Fortschritt und geplanten Todos',
    featured: true,
    intro: 'Bündel zusammengehörige Aufgaben als Projekt mit Fortschritt und geplanten Todos. So behältst du bei größeren Vorhaben den Überblick.',
    standalone: true,
    integrated: false,
    component: lazy(() => import('./projekte/TabProjekte')),
  },
  {
    id: 'growth',
    tabId: 18,
    name: 'Growth',    color: '#4ADE80',
    description: 'Tägliche Reflexionskarte & Journal — ein Satz reicht',
    featured: true,
    intro: 'Eine ruhige Reflexionskarte pro Tag — ein Satz reicht. Über die Zeit wird daraus dein persönliches Journal.',
    standalone: true,
    integrated: true,
    component: lazy(() => import('./growth/TabGrowth')),
  },
  {
    id: 'notizen',
    tabId: 19,
    name: 'Notizen',    color: '#EAB308',
    description: 'Schnelle Notizen festhalten und wiederfinden',
    standalone: true,
    integrated: false,
    component: lazy(() => import('./notizen/TabNotizen')),
  },
]

export const TOOL_TAB = Object.fromEntries(TOOL_REGISTRY.map(t => [t.id, t.tabId]))
