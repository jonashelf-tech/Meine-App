...
src/
  ...
  features/
    ...
    buddy/             — KI-Begleiter (Eichhörnchen; Konzept `Dateien/output/ki-buddy-konzept.md` — bindend)
      ...
      buddyImpuls.js          — pur: Briefkasten-Engine (Stufe 2). Lokal-deterministische Trigger (rueckkehr/verschoben/prio1/morgen/poolstau), **max. 1 Gedanke/Tag**, Backoff pro Thema, kein API-Call bei der Berechnung. Guard: buddyImpuls.test.js (Quell-Guard + Ton-Bannliste). computeImpuls/dismiss/consume/impulsTeaser + KIND_BY_TRIGGER. State `SK.buddyImpuls` (ephemer, gerätelokal, nie Sync/Backup)
      buddyApi.js             — fetch auf POST /buddy des eigenen Workers (Auth = Cloud-Token); Fehler als deutsche Sätze
      BuddyAvatar.jsx         — Pixelart-Eichhörnchen v3 (Posen idle/denkt), 48×48-Raster als Zeichenketten je Zeile + Palette; aus Jonas' Referenzbild (`Dateien/input/`) programmatisch extrahiert (Canvas-Pixel-Auslese + Farb-Clustering, `tools/shot.mjs` für die Vergleichs-Screenshots), nicht von Auge nachgezeichnet
      BuddyFab.jsx            — Avatar-Button links unten, App.jsx mountet ihn NUR auf Tab 0+1, nur wenn buddySettings.enabled. Stufe 2: berechnet beim Mount buddyImpuls (nur bei `buddySettings.gedanken`) und zeigt einen stillen Punkt am Avatar, wenn ein Gedanke bereitliegt — nie Text von selbst
      BuddySheet.jsx          — Gesprächs-Sheet: Schnellaktionen (start/zerlegen/tagesplan/ueberfordert), Action-Karten mit „Übernehmen" → bestehende Store-Pfade (setTodos/setDays/timerAutoStart). Eiserne Regel: Buddy schlägt vor, Nutzer bestätigt — keine stillen Mutationen. Stufe 2: Impuls-Banner (Zeig mir/Nicht jetzt — API-Call erst bei „Zeig mir") + `initialSend`-Prop (Direkteinstieg, z.B. „Mit {Name} klären" aus dem Prokrastination-Tool)
