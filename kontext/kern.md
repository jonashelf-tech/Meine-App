...
- **Bekannte Lücke (nicht gebaut):** Der Tagesplaner-Raster (`Zeitplan.jsx`/`Tagesliste.jsx`/`SlotBlock.jsx`, angesteuert über `useSlotMutations.js`) zeigt platzierte geteilte Termine an, kennt aber weder Emoji noch den globalen `calFilter` — ein ausgeblendeter Kalender verschwindet dort NICHT (anders als in Woche/Monat/DayPanel, wo das bewusst gebaut ist). Bestätigt im Preview: „Zahnarzt" (Familie) bleibt im Raster sichtbar, obwohl der Familie-Chip aus ist. Fix bräuchte Filterung in Zeitplan.jsx (Drag/Band-Mathematik nicht anfassen, nur Anzeige) + evtl. Emoji-Overlay am `SlotBlock`-Wrapper (nicht in `TodoChip` selbst, das ist geteilte Infrastruktur für Pool & Co.) — noch nicht umgesetzt.
