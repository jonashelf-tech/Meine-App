...
- **Notifications-Feed (A10 — ersetzt den Roadmap-Punkt „Aktivität light", Jonas 2026-07-22):** `getNotifications()` in `src/features/cal/notifications.js` (pur, `notifications.test.js`) leitet „das ist passiert seit deinem letzten Blick" ab aus `todos` + `calTombstones` + `calList` + `calCreds` + `calFilter` + `calSeen` + `days` — berechnet, nie gespeichert (kein Sync-State, kein Log). Eintrag = `{ type: added|changed|done|deleted|clash, calId, todoId, by, who, title, at, date, time, clashWith? }`, neuestes zuerst. Fenster: fremder Stempel (`by ≠` meine `memberId`) mit `updatedAt > calSeen[calId]`; Typ nach Spec §7 (`createdAt` nach dem Blick → added · frisches `doneAt` → done · sonst changed); **clash** = unplatzierter fremder Termin überlappt einen eigenen Slot (`blocksOverlap`; platziert oder erledigt = kein Konflikt), `clashWith` = Text des getroffenen Slots; **deleted** aus Tombstones — bewusst ohne Titel (der Record existiert nach dem Merge nicht mehr, dafür einen lokalen Log anzulegen wäre neuer State). Der A8-Aktivitäts-Toggle (`calFilter.cals[id].activity`) stummt einen Kalender im Feed; der Sicht-Chip (`show`) ist bewusst getrennt davon. **UI:** Signal-Feld rechts in der `CalFilterChips`-Zeile (Variante A, dezentes `sigBreath`-Glimmen + Zähl-Punkt nur bei Neuem; Ruhe = gedimmt, bleibt antippbar) → `DigestSheet` (Overlay sheet, „Das ist passiert", Gruppen nach Kalender in `calList`-Reihenfolge, Kollisions-Zeile mit Rose-Kante, Empty-State). **Gesehen global (§4-Entscheidung):** `calSeen` ist reaktiver Store-Slice (`{[calId]: ts}`, Policy ephemer — nie Server, nie Backup); das Öffnen des Digests setzt ALLE Kalender auf jetzt, das Sheet zeigt einen Snapshot. Kein Server-Push, keine OS-Notification, kein Live-Kanal — passives Glimmen statt Pop-up.
  - **Verlassener Kalender (A12-Feinschliff, 2026-07-22):** `leaveCal` entfernt nur die Creds — die Einträge bleiben lokal (`cal`-Feld erhalten, read-only, kein Datenverlust). `getNotifications` überspringt Kalender **ohne** `calCreds` (kein Mitglied mehr → kein Live-Feed); sonst tauchten nach dem Verlassen sogar EIGENE Alt-Aktionen als „neu" auf, weil `by === memberId` mit `memberId = null` nicht mehr greift. `CalFilterChips` zeigt für Kalender, die noch in `calList`, aber nicht mehr in `calCreds` stehen, einen dezenten „Kopplung getrennt — nur lesbar"-Hinweis (`.orphan`). Guard: `notifications.test.js` „verlassener Kalender".
