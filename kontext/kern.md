...
- **Geteilte Kalender im Lesepfad (A9):** Kalender-Ansichten rendern eigene Einträge weiterhin aus `days`. Geteilte Termine anderer Mitglieder liegen NUR in `todos` (`cal != null`) und kommen **additiv** dazu — `getUnplacedCalItems()` (in `kalenderShared.js`) liefert sie pro Tag, entdoppelt gegen platzierte Slots über `todoId` und ohne erledigte. Der globale `calFilter` gilt überall: `isEntryShown(calFilter, calId)` — Eintrag ohne `cal` folgt dem Privat-Chip, mit `cal` dem Kalender-Chip. Kennung ist das Kalender-Emoji (`calEmoji()`, Default `👥`): am eigenen Wochen-Block als Ecke oben rechts, am Monatsbalken/DayPanel-Eintrag vor dem Text. Geteilte Termine ohne eigenen Slot sind **read-only** (Wochen-Overlay `left:12px`, gestrichelt, `pointer-events:none`) — sie folgen allein den Kalender-Chips, nicht dem Termine/Todos/Tools-Toggle-Strip.
